...
const perform = async (ctx: RuntimeContext, bundle: Bundle): Promise<AsyncResult> => {
    ...
    if (response.status !== 200) {
        ctx.console.error(`Request failed with status ${response.status}: ${response.content}`);
        throw new Error(`Third-party service returned error (${response.status}): ${response.data?.error || response.content}`);
    }
    ...
    if (!taskId) {
        ctx.console.error('Third-party service did not return a taskId:', JSON.stringify(response.data));
        throw new Error('Third-party service did not return a taskId');
    }
...
const checkStatus = async (
    ...
    // Use statusCheckUrl endpoint which advances task state (for polling fallback)
