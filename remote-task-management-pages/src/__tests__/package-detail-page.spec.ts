...
describe('PackageDetailPage', () => {
  ...
  it('opens create-instance sheet with shared sheet structure', async () => {
    ...
    expect(wrapper.html()).toContain('fixed inset-0 z-50 flex justify-end bg-slate-950/70 backdrop-blur-sm')
    expect(document.querySelector('[data-test-id="remote-task-management.package-detail.create-instance.name"]')).toBeTruthy()
    expect(document.querySelector('[data-test-id="remote-task-management.package-detail.create-instance.description"]')).toBeTruthy()
    expect(document.querySelector('[data-test-id="remote-task-management.package-detail.create-instance.version"]')).toBeTruthy()
    expect(document.querySelector('[data-test-id="remote-task-management.package-detail.create-instance.visibility"]')).toBeTruthy()
    expect(document.querySelector('[data-test-id="remote-task-management.package-detail.create-instance.submit"]')).toBeTruthy()
  ...
  it('renders package detail panels through shared card primitives', async () => {
    ...
    expect(wrapper.text()).toContain('Package Statistics')
    expect(wrapper.text()).toContain('my instance')
