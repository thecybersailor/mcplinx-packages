...
<template>
  ...
  >
    ...
    <div v-else-if="!packages.length" class="rounded-2xl border border-dashed border-white/12 bg-white/[0.03] px-6 py-10 text-center">
      <div class="text-lg font-semibold text-white">No packages found</div>
    </div>
    <div v-else class="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      <table data-test-id="remote-task-management.packages.table" class="min-w-full bg-transparent">
        ...
        >
            ...
            <td class="px-4 py-4 text-slate-300">{{ pkg.author?.email || pkg.author?.name || '-' }}</td>
            <td class="px-4 py-4">
