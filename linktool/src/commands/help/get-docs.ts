...
export function getDocsCommand() {
    return new Command('get-docs')
        ...
        .action(async () => {
            ...
            const docsPath = join(cwd, DOCS_DIR);
            const tempZipPath = join(cwd, '.linktool', 'docs-temp.zip');

            try {
                console.log(chalk.blue('📥 Downloading documentation...'));
                console.log(chalk.gray(`URL: ${DOCS_URL}`));
                ...
                await downloadFile(DOCS_URL, tempZipPath);

                console.log(chalk.green('✓ Download complete'));

                // Extract zip file
                console.log(chalk.blue('📦 Extracting documentation...'));
                ...
                // Extract zip
                await extractZip(tempZipPath, docsPath);

                // Remove temp zip file
                await unlink(tempZipPath);

                console.log(chalk.green(`✓ Documentation extracted to ${DOCS_DIR}`));
                console.log(chalk.cyan(`\nDocumentation is now available at: ${docsPath}`));
            } catch (error: any) {
                console.error(chalk.red('Error downloading documentation:'), error.message);
                process.exit(1);
            }
        });
}
...
async function extractZip(zipPath: string, destPath: string): Promise<void> {
    ...
    try {
        if (isWindows) {
            ...
            const tempExtractPath = join(destPath, '..', 'docs-temp-extract');
