  ...
  >
    <BundlePanel>
      <form class="flex items-center gap-2" @submit.prevent="submit">
        ...
        >
        <button class="inline-flex items-center justify-center rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90" data-test-id="shared-connections.fallback.submit">
    ...
    <div v-else-if="response" class="space-y-3">
      <BundlePanel v-if="response.selected" data-test-id="shared-connections.fallback.selected">
        <div class="text-xs uppercase tracking-wide text-muted-foreground">{{ runtime.t('sharedConnections.selected', 'Selected') }}</div>
        <div class="mt-2 font-semibold text-foreground">{{ response.selected.label || response.selected.connector_id || response.selected.id }}</div>
      </BundlePanel>
      <div class="grid gap-3">
        ...
        >
          <div class="font-semibold text-foreground">{{ candidate.id }}</div>
          <div class="mt-1 text-sm leading-6 text-muted-foreground">{{ candidate.resolution_hint }}</div>
        </BundlePanel>
      </div>
    </div>
  </BundlePage>
